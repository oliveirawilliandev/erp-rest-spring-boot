...
import br.com.willian.controller.docs.FileControllerDocs; // Interface de documentação Swagger/OpenAPI
import br.com.willian.dto.v1.UploadFileResponseDTO; // DTO para resposta de upload de arquivo
import br.com.willian.service.FileStorageService; // Serviço para operações com arquivos
...
public class FileController implements FileControllerDocs { // Implementa a interface de documentação
    ...
    public ResponseEntity<Void> uploadUserPhoto(
        ...
        logger.info("[FILE-CTRL-004] Upload de foto do usuário | userId={} | fileName={}",
                id, file.getOriginalFilename());

        if (file.isEmpty()) {
            logger.warn("[FILE-CTRL-004] Arquivo vazio para usuário: {}", id);
        ...
        if (contentType == null || !contentType.startsWith("image/")) {
            logger.warn("[FILE-CTRL-004] Arquivo não é imagem | userId={} | contentType={}", id, contentType);
            return ResponseEntity.badRequest()
                    .body(null);
        }

        // VALIDAÇÃO: Aceitar apenas JPG ou JPEG
        boolean isValidJpg = contentType.equals("image/jpeg") ||
                contentType.equals("image/jpg") ||
                fileNameOriginal.toLowerCase().endsWith(".jpg") ||
                fileNameOriginal.toLowerCase().endsWith(".jpeg");

        if (!isValidJpg) {
            logger.warn("[FILE-CTRL-004] Apenas arquivos JPG são permitidos | userId={} | contentType={} | fileName={}",
                    id, contentType, fileNameOriginal);
            return ResponseEntity.badRequest()
                    .body(null);
        }

        // USA O NOVO MÉTODO específico para foto de usuário
        var fileName = fileStorageService.storeFilePhotoUser(id, file);
    ...
    // Endpoint específico para fotos de usuário
    @GetMapping("/downloadUserPhoto/{fileName:.+}")
    public ResponseEntity<Resource> downloadUserPhoto(@PathVariable String fileName, HttpServletRequest request) {

        logger.info("[FILE-CTRL-007] Download foto de usuário | fileName={}", fileName);

        // Usa o método que busca no diretório de fotos
        var resource = fileStorageService.loadUserPhotoAsResource(fileName);
        ...
        try {
            ...
            logger.warn("Não foi possível resolver o Content-Type", ex);
        }
