...
/**
 * Mapper responsável por converter objetos da entidade {@link Employee} para {@link EmployeeDTO} e vice-versa.
...
public interface EmployeeMapper {
    ...
    Logger logger = LoggerFactory.getLogger(EmployeeMapper.class); // Logger para rastreamento

    /**
     * [EMP-MAPPER-001]
     * Converte uma instância de {@link Employee} para {@link EmployeeDTO}.
     * @param employee a entidade a ser convertida
     * @return o DTO correspondente ou {@code null} se a entidade for {@code null}
     */
    EmployeeDTO toDTO(Employee employee); // Método MapStruct para conversão entidade → DTO

    /**
     * [EMP-MAPPER-002]
     * Converte uma instância de {@link EmployeeDTO} para {@link Employee}.
     * @param employeeDTO o DTO a ser convertido
     * @return a entidade correspondente ou {@code null} se o DTO for {@code null}
     */
    Employee toEntity(EmployeeDTO employeeDTO); // Método MapStruct para conversão DTO → entidade

    /**
     * [EMP-MAPPER-003]
     * Converte uma lista de entidades {@link Employee} para uma lista de {@link EmployeeDTO}.
     * @param employeeList lista de entidades
     * @return lista de DTOs correspondente ou lista vazia se a entrada for {@code null}
     */
    List<EmployeeDTO> toDTOList(List<Employee> employeeList); // Método MapStruct para conversão lista de entidades → lista de DTOs

    /**
     * [EMP-MAPPER-004]
     * Converte uma lista de {@link EmployeeDTO} para uma lista de entidades {@link Employee}.
     * @param employeeDTOList lista de DTOs
     * @return lista de entidades correspondente ou lista vazia se a entrada for {@code null}
     */
    List<Employee> toEntityList(List<EmployeeDTO> employeeDTOList); // Método MapStruct para conversão lista de DTOs → lista de entidades
    ...
    /**
     * [EMP-MAPPER-007]
     * Converte uma página de entidades {@link Employee} para uma página de {@link EmployeeDTO}.
     ...
     */
    default Page<EmployeeDTO> toDTOPage(Page<Employee> employeesPage) { // Método auxiliar para conversão de página
        logger.debug("[EMP-MAPPER-007] Convertendo Page<Employee> para Page<EmployeeDTO>"); // Log de início
        ...
        List<EmployeeDTO> dtoList = toDTOList(employeesPage.getContent()); // Converte conteúdo da página
        logger.debug("[EMP-MAPPER-007] Conteúdo convertido | dtoListSize={}", dtoList.size()); // Log do tamanho da lista convertida

        Page<EmployeeDTO> result = new PageImpl<>(
    ...
    /**
     * [EMP-MAPPER-008]
     * Converte uma página de {@link EmployeeDTO} para uma página de entidades {@link Employee}.
     ...
     */
    default Page<Employee> toEntityPage(Page<EmployeeDTO> dtoPage) { // Método auxiliar para conversão de página DTO → entidade
        logger.debug("[EMP-MAPPER-008] Convertendo Page<EmployeeDTO> para Page<Employee>"); // Log de início
        ...
        List<Employee> entityList = toEntityList(dtoPage.getContent()); // Converte conteúdo da página
        logger.debug("[EMP-MAPPER-008] Conteúdo convertido | entityListSize={}", entityList.size()); // Log do tamanho da lista convertida

        Page<Employee> result = new PageImpl<>(
