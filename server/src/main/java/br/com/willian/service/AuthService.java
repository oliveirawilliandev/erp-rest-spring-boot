...
public class AuthService {
    ...
    public ResponseEntity<TokenDTO> signIn(AccountCredentialsDTO accountCredentialsDTO) throws Exception {
        ...
        // Gera token JWT
        var token = jwtTokenProvider.createAccessToken(accountCredentialsDTO.getUserName(), user.getRoles(), user.getFullName(), user.getPhotoUrl());
    ...
    public ResponseEntity<TokenDTO> refreshToken(String userName, String refreshToken) {
        ...
        // Garantir que o fullName está presente mesmo no refresh
        if (token.getFullName() == null && user.getFullName() != null) {
    ...
    public AccountCredentialsDTO create(AccountCredentialsDTO accountCredentialsDTO) {
        ...
        entity.setEmail(accountCredentialsDTO.getEmail());
    ...
    public AccountCredentialAdminDTO createAdmin(AccountCredentialAdminDTO accountCredentialAdminDTO) {
        ...
        entity.setEmail(accountCredentialAdminDTO.getEmail());
        entity.setAccountNonExpired(true);
        entity.setAccountNonLocked(true);
        entity.setCredentialsNonExpired(true);
        entity.setEnabled(true);
    ...
    // [AUTH-SRV-006] Atualiza um registro existente
    public void update(UserEditProfileDTO userEditProfileDTO) {

        if (userEditProfileDTO == null) {
            logger.error("[AUTH-SRV-006] Attempted to update null user");
            throw new RequiredObjectIsNullException();
        }

        logger.info("[AUTH-SRV-006] Updating user with ID: {}", userEditProfileDTO.getId());

        User entityLoaded = userRepository.findById(userEditProfileDTO.getId())
                .orElseThrow(() -> {
                    logger.warn("[AUTH-SRV-006] User with ID {} not found for update", userEditProfileDTO.getId());
                    return new ResourceNotFoundException("No records found for this ID");
                });

        logger.debug("[AUTH-SRV-006] Updating user: {}", entityLoaded.getFullName());

        // Atualiza apenas os campos permitidos
        if (userEditProfileDTO.getFullName() != null && !userEditProfileDTO.getFullName().isEmpty()) {
            entityLoaded.setFullName(userEditProfileDTO.getFullName());
        }

        if (userEditProfileDTO.getEmail() != null && !userEditProfileDTO.getEmail().isEmpty()) {
            entityLoaded.setEmail(userEditProfileDTO.getEmail());
        }

        if (userEditProfileDTO.getPhotoUrl() != null && !userEditProfileDTO.getPhotoUrl().isEmpty()) {
            entityLoaded.setPhotoUrl(userEditProfileDTO.getPhotoUrl());
        }

        // Só atualiza senha se foi fornecida
        if (userEditProfileDTO.getPassword() != null && !userEditProfileDTO.getPassword().isEmpty()) {
            entityLoaded.setPassword(generateHashedPassword(userEditProfileDTO.getPassword()));
        }

        userRepository.save(entityLoaded);
        logger.info("[AUTH-SRV-006] User updated successfully | id={}", userEditProfileDTO.getId());
    }
}